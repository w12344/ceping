/**
 * QRCode.js v1.0.0 (High-Precision Floating Point Canvas Engine Fix)
 * (c) Sangmin Shim, MIT License
 */
var QRCode;
(function () {
	...
	var QRCodeLimitLength = [
		...
		[644, 504, 364, 280], [718, 558, 398, 310], [792, 608, 442, 338], [858, 666, 482, 382],
		[929, 711, 509, 403], [1003, 779, 565, 439], [1091, 851, 611, 461], [1171, 911, 661, 511],
		[1273, 985, 715, 535], [1367, 1057, 751, 593], [1465, 1125, 805, 625], [1528, 1190, 868, 658],
		[1628, 1264, 908, 698], [1732, 1370, 982, 742], [1840, 1452, 1038, 790], [1952, 1538, 1098, 842],
		[2068, 1628, 1162, 898], [2188, 1722, 1230, 948], [2303, 1809, 1283, 983], [2431, 1911, 1351, 1051],
		[2563, 1989, 1423, 1093], [2699, 2099, 1499, 1139], [2809, 2213, 1579, 1219], [2953, 2331, 1663, 1273]
	];

	function _getUTF8Length(sText) {
		var replacedText = encodeURI(sText).replace(/%[0-9A-F]{2}/g, "a");
	...
	// 浮点精准 Canvas 模块对齐，保证 Canvas 100% 满填充无无缝、无右/下切割缺边！
	Drawing.prototype.draw = function (oQRCode) {
		var _htOption = this._htOption;
		var _el = _htOption.el;
		...
		for (var row = 0; row < nCount; row++) {
			for (var col = 0; col < nCount; col++) {
				if (oQRCode.isDark(row, col)) {
					var x = col * nWidth;
					var y = row * nHeight;
					var w = (col + 1) * nWidth - x;
					var h = (row + 1) * nHeight - y;
					ctx.fillRect(x, y, w + 0.05, h + 0.05);
				}
			}
		}
		if (_el) _el.appendChild(canvas);
	};
	...
	QRCode.prototype.makeCode = function (sText) {
		this._oQRCode = new QRCodeModel(QRCode._getTypeNumber(sText, this._htOption.correctLevel), this._htOption.correctLevel);
