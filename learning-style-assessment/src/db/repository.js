...
export function createRepository(database, { allowedAnswers: definitions } = {}) {
  ...
  function listAdminSessions() {
    return database.prepare(`
      SELECT
        id, anonymous_code AS anonymousCode, report_access_token AS reportAccessToken,
        student_name AS studentName, phone_number AS phoneNumber,
        grade, score_band AS scoreBand, specialty_direction AS specialtyDirection,
        foreign_language AS foreignLanguage, target_subject AS targetSubject,
        target_subject_score AS targetSubjectScore, target_subject_full_score AS targetSubjectFullScore,
        score_level AS scoreLevel, learning_focus AS learningFocus,
        result_type AS resultType, primary_preference AS primaryPreference,
        secondary_preference AS secondaryPreference,
        started_at AS startedAt, submitted_at AS submittedAt,
        duration_seconds AS durationSeconds, report_json IS NOT NULL AS hasReport
      FROM assessment_sessions
