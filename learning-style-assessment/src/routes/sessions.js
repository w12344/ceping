...
import { uploadReportToOSS } from "../lib/oss.js";
...
export function createSessionsRouter({ repository, database, config, assessmentVersion = "v2.7.1" }) {
  const router = Router();
  ...
  router.post("/:id/submit", (request, response) => {
    ...
    const publicPayload = publicReportView(storedReport);
    if (config?.oss?.accessKeyId && config?.oss?.accessKeySecret) {
      ...
      }, publicPayload, config.oss).catch(() => {});
    }
    response.set("Cache-Control", "no-store");
    return response.status(201).json({
      anonymousCode: session.anonymousCode,
      reportUrl: `/report.html?id=${encodeURIComponent(session.reportAccessToken)}`,
      report: publicPayload
    });
  });
